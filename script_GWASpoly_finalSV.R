...
write.csv(qtl,file = paste("QTLs_sigSVdosage_",trait,".csv",sep = ""), sep = "\t")

#write output GWAS
write.GWASpoly(data=data2, trait=trait, filename= paste("GWAS_files_SV_dosage/GWAS_",trait,"_effects.csv", sep = ""), what = "effects", delim = "\t")
write.GWASpoly(data=data2, trait=trait, filename= paste("GWAS_files_SV_dosage/GWAS_",trait,"_scores.csv",sep = ""), what = "scores", delim = "\t")
